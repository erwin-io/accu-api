...
export class UsersService {
  ...
  async updateClientUser(userDto: UpdateClientUserDto) {
    ...
    return await this.userRepo.manager.transaction(async (entityManager) => {
      let client: any = await entityManager.findOne(Clients, {
        where: { user: { userId, userType: { userTypeId: "2" } } },
        relations: {
          user: {
            gatewayConnectedUsers: true,
            entityStatus: true,
  ...
  async updateStaffUser(userDto: UpdateStaffUserDto) {
    ...
    return await this.userRepo.manager.transaction(async (entityManager) => {
      let staff: Staff = await entityManager.findOne(Staff, {
        where: { user: { userId, userType: { userTypeId: "1" } } },
        relations: {
          user: {
            gatewayConnectedUsers: true,
            entityStatus: true,
